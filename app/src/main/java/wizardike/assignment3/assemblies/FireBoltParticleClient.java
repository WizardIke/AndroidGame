...
import wizardike.assignment3.physics.Collision.CollisionHandlers.ExplodeClient;
import wizardike.assignment3.physics.Collision.TriggeredCircleHitBox;

/**
 * Created by Ike on 30/01/2017.
 */
public class FireBoltParticleClient {
    ...
    public static int create(Level level, float posX, float posY, float radius, float dirX, float dirY,
                      float speed, int caster, WalkingSpriteSheet spriteSheet) {
        ...
        ExplodeClient explode = new ExplodeClient(position, dirX * speed, dirY * speed);
